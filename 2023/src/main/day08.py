...
def trace_path(instructions: str, nodes: dict[str, Node]) -> int:
    ...
    while cur_node.key != "ZZZ":
        cur_node = nodes[cur_node.left if instructions[instruction_index] == "L" else cur_node.right]
...
def find_loop_intervals(instructions: str, nodes: dict[str, Node]) -> list[int]:
    ...
    cur_nodes = list(filter(lambda node: is_start_node(node.key), nodes.values()))
