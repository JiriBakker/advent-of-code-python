...
def parse_almanac(input: list[str]) -> tuple[list[int], list[AlamanacMapping]]:
    seeds = list(map(lambda nr: int(nr), input[0].split(":")[1].strip().split(" ")))

    mappings: list[AlamanacMapping] = []

    cur_mapping = AlamanacMapping()
    for line in input[2:]:
        if len(line) == 0:
            continue
...
def find_lowest_location_with_seeds(seeds: list[int], mappings: list[AlamanacMapping]) -> int:
...
def find_lowest_location_with_seed_ranges(seed_ranges: list[tuple[int, int]], mappings: list[AlamanacMapping]) -> int:
    location = 0
    while True:
        cur_value = location
        for mapping in mappings:
